...
export const TradingChartOverlay = ({ smcData }: TradingChartOverlayProps) => {
  ...
  return (
    <>
      {/* Backdrop clicável para fechar no mobile */}
      {isVisible && (
        <div 
          className="absolute inset-0 z-5 bg-black/20 md:bg-transparent"
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-2">
        {/* Botão Toggle - área de toque maior para mobile */}
        <Button
          onClick={() => setIsVisible(!isVisible)}
          variant="secondary"
          size="sm"
          className="gap-2 shadow-lg min-h-[44px] min-w-[44px] touch-manipulation"
        >
          {isVisible ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
          <span className="font-semibold hidden sm:inline">SMC</span>
        ...
        {isVisible && hasStructures && (
          <div className="bg-background/95 backdrop-blur-sm border border-border rounded-lg p-3 shadow-lg w-[280px] sm:max-w-xs transition-all duration-300 animate-scale-in">
            {/* Header com botão X visível */}
            <div className="flex items-center justify-between gap-2 pb-2 border-b border-border">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                <span className="font-bold text-foreground text-xs">ESTRUTURAS SMC ATIVAS</span>
              </div>
              <Button
                onClick={() => setIsVisible(false)}
                variant="ghost"
                size="sm"
                className="h-8 w-8 p-0 min-h-[44px] min-w-[44px] -mr-1 touch-manipulation hover:bg-destructive/10"
